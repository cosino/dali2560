...
VARIANT = getBoardConf(r'^%s\.build\.variant=(.*)'%ARDUINO_BOARD)

ARDUINO_CORE = pathJoin(ARDUINO_HOME, 'hardware/arduino/cores/arduino')
ARDUINO_VARS = pathJoin(ARDUINO_HOME, 'hardware/arduino/variants/' + VARIANT)
...
envArduino = Environment(CC = AVR_BIN_PREFIX+'gcc', CXX = AVR_BIN_PREFIX+'g++',
    CPPPATH = ['.', 'build/core', 'build/variants'],
...
VariantDir('build/variants', ARDUINO_VARS)
